...
**dev 目标配置：**2025 款中国版 BYD 海狮 07 EV，610 智驾版，后轮驱动。
...
BYDMate 是一款运行在 BYD DiLink 车机上的 Android 应用。此 dev 分支面向 2025 款中国版海狮 07 EV 610 智驾版后驱车型。它可以记录行程、GPS 路线、BMS 真实能耗、充电记录，并提供 AI 驱动的驾驶分析。无需 Google Play Services。
...
当前 dev 版本的语音导航统一使用 Waze；另外还包含"语音短语"自动化触发器，以及在车机本地计算的每周/每月洞察。
...
**Waze 投射到仪表盘。** 学习指定方向盘按键后，可将导航投射到仪表盘或切回中控屏。dev 版本默认投射 Waze。详见“投射到仪表盘”章节。
...
| **Cluster** | 投射到仪表盘 | 通过已学习的方向盘按键将所选应用投射到仪表盘 |
...
- **短按所选按键**：将应用投射到仪表盘。
- **再次短按**：切回中控屏。
- **如果分配了右星键**：长按仍保持原车行为（车机菜单）；BYDMate 不会拦截。

海狮 07 配置默认不分配按键，因此 BYDMate 不会拦截未经实车验证的按键代码。
...
之后请在此处学习一个按键；短按该按键即可将所选应用投射到仪表盘或切回中控屏。

### 投射哪个应用

默认投射 Waze。投射应用选择器仍可选择任意已安装应用，但 BYDMate 的路线命令、导航自动化和 HUD 数据只使用 Waze；未添加 Google Maps 导航提供程序。新选择在下次按下星键时生效。

### 窗口大小和位置

在 **设置 → 显示 → "仪表盘窗口大小"** 中，五个滑块分别控制宽度和高度（20–100%）、水平和垂直位置（0–100%）以及内容缩放（50–150%）。此 dev 版本默认使用海狮 07 小窗口预设 **32 / 92 / 100 / 6 / 90**；传统全屏仪表可使用 **100 / 100 / 50 / 50 / 100**。系统投射容器默认不自动开启，请先在实车上确认窗口几何和容器兼容性。
...
1. **车载系统的能量流状态**：gun-state 2 = AC，3 或 4 = DC。状态 5 是 V2L/VTOL 对外放电，不属于充电。若该信号不可用，则进入下一步。
2. **会话平均功率**：大于 15 kW = DC，否则为 AC。这只是备用启发式规则，并非 GB/T 协议边界；低功率或接近结束的 DC 会话偶尔可能被判断为 AC，可手动修正记录。
...
此 dev 分支已配置为 **2025 款中国版 BYD 海狮 07 EV，610 智驾版，RWD**。底层 FID/SOME-IP 协议、SoH 和自动充电记录最初均在钛 3 上验证，因此在海狮 07 上仍需逐项实车测试。

- **默认电池容量：**80.64 kWh 标称容量。可用容量和电池包标称电压保持未知，不从平台名称推测。
- **能耗阈值：**18/24 kWh/100 km 仅为界面颜色启发式阈值，不是官方 15.2 kWh/100 km 标定能耗。
- **行程记录：**`energydata` 可用时使用 BMS 数据，否则由实时遥测记录。
- **SoH、充电、车辆命令和 HUD：**相应实车测试通过前，均视为未验证。
...
| 参数 | 值 |
|----------|-------|
| 车辆 | 2025 款 BYD 海狮 07 EV，中国版 |
| 配置 / 型号 | 610 智驾版; BYD6486SBEV2 |
| 驱动形式 | 后单电机，RWD；230 kW；380 N·m |
| 电池 / 架构 | LFP 刀片电池；80.64 kWh 标称；800 V 架构 |
| 续航 / 标定能耗 | 610 km CLTC；15.2 kWh/100 km |
| 整备质量 | 2210 kg |
| 尺寸 / 轴距 / 座位 | 4830×1925×1620 mm；轴距 2930 mm；5 座 |
| 总质量 / 最高车速 | 2585 kg；225 km/h |
| 轮胎 / 胎压 | 前 235/50 R19；后 255/45 R19；舒适 250 kPa / 经济 290 kPa |
| 充电标准 | GB/T（中国） |
| ADAS | DiPilot 300；1 个激光雷达 |
| 车机 / 固件 | 实车测试时记录；不假定钛 3 的硬件参数 |
...
在 **设置** 中可以修改：
- **电池容量** — 默认 80.64 kWh（海狮 07 EV 610 智驾版后驱）
- **电价** — 家庭充电（AC）和快充（DC），货币
- **能耗阈值** — 18/24 界面颜色启发式阈值，非官方标定能耗
