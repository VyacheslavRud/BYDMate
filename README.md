...
- **История поездок**: BYDMate сейчас читает поездки из встроенной BMS-базы `energydata`, которая есть на Leopard 3 (Fangchengbao Bao 3). На моделях без этой базы (Song, Yuan и аналоги) список поездок будет пустой — поддержка таких машин планируется в одном из ближайших обновлений.
