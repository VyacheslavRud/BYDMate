...
**Целевая dev-конфигурация:** BYD Sea Lion 07 EV 2025 (Китай), 610 Zhijia / 610智驾版, задний привод.
...
**Waze на приборку.** Выбранной кнопкой руля навигация переносится на приборную панель и обратно на центральный экран. В Sea Lion dev-сборке кнопка сначала назначается обучением, а навигацией по умолчанию служит Waze. Подробнее в разделе «Вывод на приборку».
...
| **Cluster** | Вывод на приборку | Вывод выбранного приложения на приборную панель обученной кнопкой руля |
...
- **Если назначена правая звезда:** её удержание работает штатно (меню машины), BYDMate его не перехватывает.

В Sea Lion-профиле кнопка по умолчанию не назначена: приложение не перехватывает непроверенный код. Назначьте её один раз, см. «Выбор кнопки» ниже.
...
После этого назначьте кнопку, затем её короткое нажатие будет переносить выбранное приложение на приборку и обратно.

### Выбор кнопки

Под тумблером нажмите «Изменить» и нажмите нужную кнопку на руле — приложение её запомнит. Назначить можно любую кнопку руля, которую видит система. Системные кнопки (громкость, питание и т.п.), обзор 360 и карусель приборки назначить нельзя.

### Какое приложение проецировать

По умолчанию на приборку выводится Waze. Под тумблером включения остаётся универсальный выбор приложения для самой проекции, но все навигационные команды BYDMate, маршруты автоматизаций и данные HUD работают только с Waze. Google Maps как навигационный провайдер не добавлен. Новый выбор приложения для проекции применяется при следующем нажатии кнопки.

### Размер и положение окна

В разделе **Настройки → Дисплей → «Размер окна на приборке»** пять ползунков. В этой dev-сборке стартовые значения настроены под мини-зону Sea Lion 07: **32 / 92 / 100 / 6 / 90**. Для обычного полноэкранного режима используйте **100 / 100 / 50 / 50 / 100**.
...
Стартовый профиль Sea Lion 07: **ширина 32%, высота 92%, сдвиг по горизонтали 100%, сдвиг по вертикали 6%, масштаб 90%**. Автоматическое включение системного контейнера по умолчанию выключено; точную геометрию и сам контейнер нужно подтвердить на вашей прошивке в машине.

<img src="docs/screenshots/cluster-mini-settings.jpg" alt="Настройки окна для Sea Lion 07" width="800">

<img src="docs/screenshots/cluster-mini-view.jpg" alt="Пример навигации в мини-зоне приборки Sea Lion 07 из предыдущей версии" width="800">

Скриншот мини-зоны сделан на предыдущей версии; текущая dev-сборка запускает в этой зоне Waze.
...
Проекция идёт через системный сервис автомобиля: BYDMate запускает выбранное приложение отдельным окном прямо на дисплее приборной панели, поэтому голосовой агент и HUD видят маршрут во время проекции. Для этого режима нужна однократная перезагрузка головного устройства после установки (долгое зажатие колёсика громкости); до неё автоматически используется старый способ через виртуальный дисплей. Чтобы перехватывать кнопки руля, приложение включает собственный сервис специальных возможностей (accessibility). Он работает только при включённом тумблере и нужен исключительно для выбранной кнопки. Прошивку и саму машину это не меняет, всё обратимо.

### Waze и заводской HUD

Для маршрутов должен быть установлен Waze с Android-пакетом `com.waze`. Голосовые команды «поехали…», поиск адреса, точки «дом/работа» и навигационные действия автоматизаций открываются только в Waze; скрытого перехода на Яндекс Навигатор или Google Maps нет.

Подсказки HUD читаются из доступного Android-дерева активного маршрута Waze: манёвр, расстояние, улица, оставшийся путь/время и ограничение скорости. На эмуляторе можно проверить запуск Waze и чтение этих полей, но сам заводской HUD проверяется только в машине. Для него в комплектации должен присутствовать системный SOME/IP-сервис; совместимость его протокола с Sea Lion 07 требует отдельного теста в автомобиле. В настройках теперь различаются отсутствие поддержки, ошибка подключения и отказ отправки кадра — это поможет понять, на каком этапе возникает проблема.
...
- Сначала в **Настройки → Голос → Назначить кнопку** обучите нужную кнопку Sea Lion, затем нажимайте её для разговора (перехват требует службы специальных возможностей, она включается автоматически).
...
- «Выведи навигацию на приборку», «Убери с приборки» (проекция Waze)
...
- Код голосовой кнопки Sea Lion заранее не предполагается: назначьте кнопку обучением непосредственно в машине.
...
1. **Тип потока энергии** от бортовой системы: gun-state 2 = AC, 3 или 4 = DC. Состояние 5 — V2L/VTOL, то есть машина отдаёт энергию наружу; оно не считается зарядкой. Если сигнал недоступен, работает следующий пункт.
2. **Средняя мощность сессии**: больше 15 кВт = DC, иначе AC. Это запасная эвристика, а не граница стандарта GB/T: медленная или сильно замедлившаяся DC-сессия иногда может определиться как AC, и запись тогда можно исправить вручную.
...
## Целевой профиль и статус совместимости

Эта dev-ветка настроена под **BYD Sea Lion 07 EV 2025, 610 Zhijia / 610智驾版, RWD (Китай)**. Низкоуровневые FID/SOME-IP-контракты, SoH и автозапись зарядок изначально были проверены на Leopard 3, поэтому на Sea Lion их нужно проверять поэтапно в машине.

- **Ёмкость батареи по умолчанию:** 80,64 кВт·ч (номинальная паспортная). Usable capacity и номинальное напряжение пакета не придумываются и в профиле остаются неизвестными.
- **Пороги расхода:** 18/24 кВт·ч/100 км — только UX-эвристика для цветов интерфейса, не паспортный расход 15,2.
- **История поездок:** при доступной `energydata` данные берутся из BMS; иначе используется нативная запись из живой телеметрии.
- **SoH, зарядки, команды и HUD:** до полевого теста на Sea Lion считаются неподтверждёнными.
...
| Параметр | Значение |
|----------|----------|
| Автомобиль | BYD Sea Lion 07 EV, 2025, China |
| Комплектация / код | 610 Zhijia / 610智驾版; BYD6486SBEV2 |
| Тяговая схема | Один задний мотор, RWD; 230 кВт; 380 Н·м |
| Батарея / архитектура | LFP Blade; 80,64 кВт·ч nominal; 800 V architecture |
| Запас хода / расход | 610 км CLTC; 15,2 кВт·ч/100 км |
| Снаряжённая масса | 2210 кг |
| Размеры / база / мест | 4830×1925×1620 мм; база 2930 мм; 5 мест |
| Полная масса / максимальная скорость | 2585 кг; 225 км/ч |
| Шины / давление | 235/50 R19 спереди; 255/45 R19 сзади; 250 кПа comfort / 290 кПа economy |
| Зарядный стандарт | GB/T (China) |
| ADAS | DiPilot 300; 1 lidar |
| Головное устройство / прошивка | Фиксируются при тесте в машине; параметры Leopard 3 не предполагаются |
...
- **Ёмкость батареи** — по умолчанию 80,64 кВт·ч (Sea Lion 07 EV 610 Zhijia RWD)
- **Тарифы** — домашний (AC) и быстрая зарядка (DC), валюта
- **Пороги расхода** — UX-эвристика 18/24 для цветовой индикции, не паспортный расход
...
- **@RBGboost** - реверс протокола SOME/IP заводского HUD и исходная реализация YandexHUD. BYDMate сохраняет созданную им транспортную и protobuf-основу, а текущая dev-сборка подаёт в неё данные Waze. Поблагодарить можно в Telegram: [@RBGboost](https://t.me/RBGboost)
